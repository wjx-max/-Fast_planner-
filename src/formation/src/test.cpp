...
{   
    ...
    double cur3_pose_x ,cur3_pose_y, cur3_pose_yaw;
     cur2_pose_x=msg->pose.pose.position.x-0.3;
     cur2_pose_y=msg->pose.pose.position.y+0.5;
     cur2_pose_yaw=0.8;

     cur3_pose_x=msg->pose.pose.position.x-0.3;
     cur3_pose_y=msg->pose.pose.position.y-0.5;
     cur3_pose_yaw=0.8;


    //发布跟随者实时位置
    geometry_msgs::PoseStamped cur2_msg; 
    cur2_msg.header.frame_id = "map";
    ...
    cur2_msg.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(0,0,cur2_pose_yaw);//cur1_pose_yaw
    //如果需要变需要要转换
    ...
    cur3_msg.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(0,0,cur3_pose_yaw);//cur1_pose_yaw
...
void AssembledFormation(){
    //----------//
    std::cout << " AssembledFormation" << std::endl;
    double cur1_pose_x, cur1_pose_y,cur1_pose_yaw;
    double cur2_pose_x, cur2_pose_y,cur2_pose_yaw;
    double cur3_pose_x, cur3_pose_y,cur3_pose_yaw;
   //跟随者需要根据领航者起始点和队形计算位置关系
    geometry_msgs::PoseStamped cur1_msg; 
    cur1_msg.header.frame_id = "map";
    cur1_msg.header.stamp = ros::Time::now();  //当前的时间，最好用领航者的时间戳
    cur1_msg.pose.position.x = 1;//cur1_pose_x;
    cur1_msg.pose.position.y = 0.1;//cur1_pose_y;
    cur1_msg.pose.position.z = 0;
    cur1_msg.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(0,0,cur1_pose_yaw);//cur1_pose_yaw

    geometry_msgs::PoseStamped cur2_msg; 
    cur2_msg.header.frame_id = "map";
    cur2_msg.header.stamp = ros::Time::now();  //当前的时间，最好用领航者的时间戳
    cur2_msg.pose.position.x = 0.5;//cur2_pose_x;
    cur2_msg.pose.position.y = 0.5;//cur2_pose_y;
    cur2_msg.pose.position.z = 0;//
    cur2_msg.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(0,0,cur2_pose_yaw);//cur2_pose_yaw

    geometry_msgs::PoseStamped cur3_msg; 
    cur3_msg.header.frame_id = "map";
    cur3_msg.header.stamp = ros::Time::now();  //当前的时间，最好用领航者的时间戳
    cur3_msg.pose.position.x = 0.5;//cur2_pose_x;
    cur3_msg.pose.position.y = -0.5;//cur2_pose_y;
    cur3_msg.pose.position.z = 0;
    cur3_msg.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(0,0,cur3_pose_yaw);//cur3_pose_yaw
    int i = 0;
    while(i < 2){
...
void PubLeadPosition(){
    ...
    cur1_msg.header.frame_id = "map";
    cur1_msg.header.stamp = ros::Time::now();  //当前的时间，最好用领航者的时间戳
    cur1_msg.pose.position.x = 4;//cur1_pose_x;
    cur1_msg.pose.position.y = 0.1;//cur1_pose_y;
    cur1_msg.pose.position.z = 0;//cur1_pose_z;
    cur1_msg.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(0,0,1);//cur1_pose_yaw
    int i = 0;
    while(i < 2){
        cur1_pub.publish(cur1_msg);
        ros::Duration(0.1).sleep();
        i++;
        std::cout << "PubLeadPosition pub............. " << std::endl;
    }
    //开始跟随标志位
    is_start_ok_ = true;
}
...
int main(int argc, char *argv[])
{
    ...
    ros::Subscriber sub = nh.subscribe("/ares1/amcl_pose", 10, LeaderPoseCallback);//队列长度:1000或1或其他

    ros::Subscriber sub1 = nh.subscribe("/ares1/odom", 10, ares1OdomCallback);//队列长度:1000或1或其他
    ros::Subscriber sub2= nh.subscribe("/ares2/odom", 10, ares2OdomCallback);//队列长度:1000或1或其他
    ros::Subscriber sub3= nh.subscribe("/ares3/odom", 10, ares3OdomCallback);//队列长度:1000或1或其他
    
    cur1_pub = nh.advertise<geometry_msgs::PoseStamped>("/ares1/move_base_simple/goal", 1); //initialize 
    cur2_pub = nh.advertise<geometry_msgs::PoseStamped>("/ares2/move_base_simple/goal", 1); //initialize 
    cur3_pub = nh.advertise<geometry_msgs::PoseStamped>("/ares3/move_base_simple/goal", 1); //initialize 
