...
//     MX x = MX::sym("x");        // x坐标
//     MX y = MX::sym("y");        // y坐标
//     MX theta = MX::sym("theta"); // 方向角
//     MX state_vars = MX::vertcat({x, y, theta});

//     // 控制变量
//     MX v_r = MX::sym("v_r"); // 右轮速度
//     MX v_l = MX::sym("v_l"); // 左轮速度
//     MX control_vars = MX::vertcat({v_r, v_l});

//     // 轮间距已知
//     const double L = 0.28; // 轮间距 (单位: 米)
...
//     // RHS (右侧方程)
//     MX rhs = MX::vertcat({
//         v * MX::cos(theta),  // dx/dt
//         v * MX::sin(theta),  // dy/dt
//         w                    // dtheta/dt
//     });
//     // 返回运动学方程的函数
//     return Function("kinematic_equation", {state_vars, control_vars}, {rhs});
...
bool Mpc::solve(Eigen::Vector3d current_states, Eigen::MatrixXd desired_states) {
    ...
    //kinematic constrains —— 之前只考虑了x方向的状态变化
    for (int i = 0; i < N_; ++i) {
        vector<MX> input(2);
        input[0] = X(all, i);
        input[1] = U(all, i);
