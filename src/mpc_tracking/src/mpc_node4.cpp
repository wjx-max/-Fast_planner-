...
ros::Publisher cmd_vel_pub4, motion_path_pub4, predict_path_pub4;
...
ros::Timer control_cmd_pub4, path_pub4;
...
void publish_control_cmd(const ros::TimerEvent &e) {
   ...
   // cout << "solve time:" << t_cur1 << endl;
    ...
    cmd_vel_pub4.publish(cmd);
    //cout << "u:" << result[0] << " " << "r:" << result[1] << endl;

    predict_path.header.frame_id = "tb3_4/odom";
    ...
    predict_path_pub4.publish(predict_path);
...
{
    ros::init(argc, argv, "mpc_tracking_node4");
    ros::NodeHandle nh;
    mpc_ptr.reset(new Mpc());

    cmd_vel_pub4 = nh.advertise<geometry_msgs::Twist>("tb3_4/cmd_vel", 1);
    predict_path_pub4 = nh.advertise<nav_msgs::Path>("tb3_4/predict_path", 1);
    motion_path_pub4 = nh.advertise<nav_msgs::Path>("tb3_4/motion_path", 1);
   
    ros::Subscriber odom_sub4 = nh.subscribe("tb3_4/odom", 1, &odomCallback);
    ros::Subscriber bspline_sub4 = nh.subscribe("tb3_4/planning/bspline", 10, bsplineCallback);
    ros::Subscriber replan_sub4 = nh.subscribe("tb3_4/planning/replan", 10, replanCallback);

    control_cmd_pub4 = nh.createTimer(ros::Duration(0.1), publish_control_cmd);
