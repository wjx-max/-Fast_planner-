...
ros::Publisher cmd_vel_pub6, motion_path_pub6, predict_path_pub6;
...
ros::Timer control_cmd_pub6, path_pub6;
...
void publish_control_cmd(const ros::TimerEvent &e) {
   ...
   // cout << "solve time:" << t_cur1 << endl;
    ...
    cmd_vel_pub6.publish(cmd);
    //cout << "u:" << result[0] << " " << "r:" << result[1] << endl;

    predict_path.header.frame_id = "tb3_6/odom";
    ...
    predict_path_pub6.publish(predict_path);
...
{
    ros::init(argc, argv, "mpc_tracking_node6");
    ros::NodeHandle nh;
    mpc_ptr.reset(new Mpc());

    cmd_vel_pub6 = nh.advertise<geometry_msgs::Twist>("tb3_6/cmd_vel", 1);
    predict_path_pub6 = nh.advertise<nav_msgs::Path>("tb3_6/predict_path", 1);
    motion_path_pub6 = nh.advertise<nav_msgs::Path>("tb3_6/motion_path", 1);
   
    ros::Subscriber odom_sub6 = nh.subscribe("tb3_6/odom", 1, &odomCallback);
    ros::Subscriber bspline_sub6 = nh.subscribe("tb3_6/planning/bspline", 10, bsplineCallback);
    ros::Subscriber replan_sub6 = nh.subscribe("tb3_6/planning/replan", 10, replanCallback);

    control_cmd_pub6 = nh.createTimer(ros::Duration(0.1), publish_control_cmd);
