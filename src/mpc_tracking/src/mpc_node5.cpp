...
ros::Publisher cmd_vel_pub5, motion_path_pub5, predict_path_pub5;
...
ros::Timer control_cmd_pub5, path_pub5;
...
void publish_control_cmd(const ros::TimerEvent &e) {
   ...
   // cout << "solve time:" << t_cur1 << endl;
    ...
    cmd_vel_pub5.publish(cmd);
    //cout << "u:" << result[0] << " " << "r:" << result[1] << endl;

    predict_path.header.frame_id = "tb3_5/odom";
    ...
    predict_path_pub5.publish(predict_path);
...
{
    ros::init(argc, argv, "mpc_tracking_node5");
    ros::NodeHandle nh;
    mpc_ptr.reset(new Mpc());

    cmd_vel_pub5 = nh.advertise<geometry_msgs::Twist>("tb3_5/cmd_vel", 1);
    predict_path_pub5 = nh.advertise<nav_msgs::Path>("tb3_5/predict_path", 1);
    motion_path_pub5 = nh.advertise<nav_msgs::Path>("tb3_5/motion_path", 1);
   
    ros::Subscriber odom_sub5 = nh.subscribe("tb3_5/odom", 1, &odomCallback);
    ros::Subscriber bspline_sub5 = nh.subscribe("tb3_5/planning/bspline", 10, bsplineCallback);
    ros::Subscriber replan_sub5 = nh.subscribe("tb3_5/planning/replan", 10, replanCallback);

    control_cmd_pub5 = nh.createTimer(ros::Duration(0.1), publish_control_cmd);
