...
ros::Publisher cmd_vel_pub1, motion_path_pub1, predict_path_pub1;
...
ros::Timer control_cmd_pub1, path_pub1;
...
void publish_control_cmd(const ros::TimerEvent &e) {
    ...
    saveToCSV(desired_state, current_state, "/home/wjx/nrypp_ws/src/mpc_tracking/desired_current_state31.csv");
   ...
   // cout << "solve time:" << t_cur1 << endl;
    ...
    cmd_vel_pub1.publish(cmd);
    //cout << "u:" << result[0] << " " << "r:" << result[1] << endl;

    predict_path.header.frame_id = "tb3_1/odom";
    ...
    predict_path_pub1.publish(predict_path);
...
{
    ros::init(argc, argv, "mpc_tracking_node1");
    ros::NodeHandle nh;
    mpc_ptr.reset(new Mpc());

    cmd_vel_pub1 = nh.advertise<geometry_msgs::Twist>("tb3_1/cmd_vel", 1);
    predict_path_pub1 = nh.advertise<nav_msgs::Path>("tb3_1/predict_path", 1);
    motion_path_pub1 = nh.advertise<nav_msgs::Path>("tb3_1/motion_path", 1);
   
    ros::Subscriber odom_sub1 = nh.subscribe("tb3_1/odom", 1, &odomCallback);
    ros::Subscriber bspline_sub1 = nh.subscribe("tb3_1/planning/bspline", 10, bsplineCallback);
    ros::Subscriber replan_sub1 = nh.subscribe("tb3_1/planning/replan", 10, replanCallback);

    control_cmd_pub1 = nh.createTimer(ros::Duration(0.1), publish_control_cmd);
