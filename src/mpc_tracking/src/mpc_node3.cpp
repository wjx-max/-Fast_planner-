...
ros::Publisher cmd_vel_pub3, motion_path_pub3, predict_path_pub3;
...
ros::Timer control_cmd_pub3, path_pub3;
...
void publish_control_cmd(const ros::TimerEvent &e) {
    ...
    saveToCSV(desired_state, current_state, "/home/wjx/nrypp_ws/src/mpc_tracking/desired_current_state33.csv");
   ...
   // cout << "solve time:" << t_cur1 << endl;
    ...
    cmd_vel_pub3.publish(cmd);
    //cout << "u:" << result[0] << " " << "r:" << result[1] << endl;

    predict_path.header.frame_id = "tb3_3/odom";
    ...
    predict_path_pub3.publish(predict_path);
...
{
    ros::init(argc, argv, "mpc_tracking_node3");
    ros::NodeHandle nh;
    mpc_ptr.reset(new Mpc());

    cmd_vel_pub3 = nh.advertise<geometry_msgs::Twist>("tb3_3/cmd_vel", 1);
    predict_path_pub3 = nh.advertise<nav_msgs::Path>("tb3_3/predict_path", 1);
    motion_path_pub3 = nh.advertise<nav_msgs::Path>("tb3_3/motion_path", 1);
   
    ros::Subscriber odom_sub3 = nh.subscribe("tb3_3/odom", 1, &odomCallback);
    ros::Subscriber bspline_sub3 = nh.subscribe("tb3_3/planning/bspline", 10, bsplineCallback);
    ros::Subscriber replan_sub3 = nh.subscribe("tb3_3/planning/replan", 10, replanCallback);

    control_cmd_pub3 = nh.createTimer(ros::Duration(0.1), publish_control_cmd);
