...
void qrviz::Set_Goal_Pose()
{
     ...
     //设置发布导航目标点的topic
     pro->subProp("Topic")->setValue("/ares1/move_base_simple/goal");
     //设置当前使用的工具
     tool_manager_->setCurrentTool(current_tool_);
}
...
void qrviz::Display_ares1_RobotModel(QString robot_description,bool enable)
{
    if(ares1_RobotModel_!=NULL)
    {
        delete ares1_RobotModel_;
        ares1_RobotModel_=NULL;
    }
    ares1_RobotModel_=manager_->createDisplay("rviz/RobotModel","Qrviz ares1_RobotModel",enable);
    ares1_RobotModel_->subProp("Robot Description")->setValue("/ares1/robot_description");
    ares1_RobotModel_->subProp("TF Prefix")->setValue("/ares1");
    ROS_ASSERT(ares1_RobotModel_!=NULL);
}

void qrviz::Display_ares2_RobotModel(bool enable)
{
    if(ares2_RobotModel_!=NULL)
    {
        delete ares2_RobotModel_;
        ares2_RobotModel_=NULL;
    }
    ares2_RobotModel_=manager_->createDisplay("rviz/RobotModel","Qrviz ares2_RobotModel",enable);
    ares2_RobotModel_->subProp("Robot Description")->setValue("/ares2/robot_description");
    ares2_RobotModel_->subProp("TF Prefix")->setValue("/ares2");
    ROS_ASSERT(ares2_RobotModel_!=NULL);
}

void qrviz::Display_ares3_RobotModel(bool enable)
{
    if(ares3_RobotModel_!=NULL)
    {
        delete ares3_RobotModel_;
        ares3_RobotModel_=NULL;
    }
    ares3_RobotModel_=manager_->createDisplay("rviz/RobotModel","Qrviz ares3_RobotModel",enable);
    ares3_RobotModel_->subProp("Robot Description")->setValue("/ares3/robot_description");
    ares3_RobotModel_->subProp("TF Prefix")->setValue("/ares3");
    ROS_ASSERT(ares3_RobotModel_!=NULL);
}
