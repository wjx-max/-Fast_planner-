...
//    ui.treeWidget->setWindowIcon(QIcon("://images/classes/Displays.svg"));
    ...
    Laser_Topic_box->addItem("ares1/scan");
    ...
    //RobotModel1
    QTreeWidgetItem* ares1_RobotModel=new QTreeWidgetItem(QStringList()<<"ares1_RobotModel");
    //设置图标
    ares1_RobotModel->setIcon(0,QIcon("://images/classes/RobotModel.png"));
    //checkbox
    QCheckBox* ares1_RobotModel_Check=new QCheckBox();
    connect(ares1_RobotModel_Check,SIGNAL(stateChanged(int)),this,SLOT(slot_display_ares1_RobotModel(int)));
    //向Treewidget添加TF Top节点
    ui.treeWidget->addTopLevelItem(ares1_RobotModel);
    //向TF添加checkbox
    ui.treeWidget->setItemWidget(ares1_RobotModel,1,ares1_RobotModel_Check);
    //ares1 topic
    QTreeWidgetItem* ares1Topic=new QTreeWidgetItem(QStringList()<<"Robot Description");
    ares1_Topic_box=new QComboBox();
    ares1_Topic_box->addItem("/ares1/robot_description");
    ares1_Topic_box->setEditable(true);
    ares1_Topic_box->setMaximumWidth(150);
    ares1_RobotModel->addChild(ares1Topic);
    ui.treeWidget->setItemWidget(ares1Topic,1,ares1_Topic_box);

    //RobotModel2
    QTreeWidgetItem* ares2_RobotModel=new QTreeWidgetItem(QStringList()<<"ares2_RobotModel");
    //设置图标
    ares2_RobotModel->setIcon(0,QIcon("://images/classes/RobotModel.png"));
    //checkbox
    QCheckBox* ares2_RobotModel_Check=new QCheckBox();
    connect(ares2_RobotModel_Check,SIGNAL(stateChanged(int)),this,SLOT(slot_display_ares2_RobotModel(int)));
    //向Treewidget添加TF Top节点
    ui.treeWidget->addTopLevelItem(ares2_RobotModel);
    //向TF添加checkbox
    ui.treeWidget->setItemWidget(ares2_RobotModel,1,ares2_RobotModel_Check);


    //RobotModel3
    QTreeWidgetItem* ares3_RobotModel=new QTreeWidgetItem(QStringList()<<"ares3_RobotModel");
    //设置图标
    ares3_RobotModel->setIcon(0,QIcon("://images/classes/RobotModel.png"));
    //checkbox
    QCheckBox* ares3_RobotModel_Check=new QCheckBox();
    connect(ares3_RobotModel_Check,SIGNAL(stateChanged(int)),this,SLOT(slot_display_ares3_RobotModel(int)));
    //向Treewidget添加TF Top节点
    ui.treeWidget->addTopLevelItem(ares3_RobotModel);
    //向TF添加checkbox
    ui.treeWidget->setItemWidget(ares3_RobotModel,1,ares3_RobotModel_Check);
    ...
    Path_Topic_box->addItem("move_base1/DWAPlannerROS/local_plan");
    ...
    //按钮快捷启动
    connect(ui.pushButton_gazebo,SIGNAL(clicked()),this,SLOT(slot_quick_gazebo_clicked()));
    connect(ui.pushButton_rviz,SIGNAL(clicked()),this,SLOT(slot_quick_rviz_clicked()));
    connect(ui.pushButton_Assem,SIGNAL(clicked()),this,SLOT(slot_quick_Assem_clicked()));

    //多机编队快速启动
    connect(ui.pushButton_leader,SIGNAL(clicked()),this,SLOT(slot_quick_leader_clicked()));
    connect(ui.pushButton_virtual,SIGNAL(clicked()),this,SLOT(slot_quick_virtual1_clicked()));
    connect(ui.pushButton_Con,SIGNAL(clicked()),this,SLOT(slot_quick_con_clicked()));

    //退出界面时关闭所有rosnode 的方法
    connect(ui.pushButton_quit,SIGNAL(clicked()),this,SLOT(slot_quick_quit1_clicked()));

    //关闭制定launch文件
    connect(ui.pushButton_stop1,SIGNAL(clicked()),this,SLOT(slot_quick_stop1_clicked()));
    connect(ui.pushButton_stop2,SIGNAL(clicked()),this,SLOT(slot_quick_stop2_clicked()));
    connect(ui.pushButton_stop3,SIGNAL(clicked()),this,SLOT(slot_quick_stop3_clicked()));
...
void MainWindow::slot_display_ares1_RobotModel(int state)
{
    bool enable=state>1?true:false;
   // myrviz->Display_ares1_RobotModel(enable);
    myrviz->Display_ares1_RobotModel(ares1_Topic_box->currentText(),enable);
}

void MainWindow::slot_display_ares2_RobotModel(int state)
{
    bool enable=state>1?true:false;
    myrviz->Display_ares2_RobotModel(enable);
}

void MainWindow::slot_display_ares3_RobotModel(int state)
{
    bool enable=state>1?true:false;
    myrviz->Display_ares3_RobotModel(enable);
}
...
//{
    ...
    //connect(laser_cmd,SIGNAL(readyReadStandardError(virtual1)),this,SLOT(slot_quick_output()));
    //connect(laser_cmd,SIGNAL(readyReadStandardOutput()),this,SLOT(slot_quick_output()));

//}

void MainWindow::slot_quick_gazebo_clicked(){
    gazebo=new QProcess;
    gazebo->start("bash");
    gazebo->write("roslaunch ares_gazebo ares_wjx_gazebo.launch\n");
}

void MainWindow::slot_quick_rviz_clicked(){
    rviz=new QProcess;
    rviz->start("bash");
    rviz->write("roslaunch ares_navigation navigation_wjx2.launch\n");
}

void MainWindow::slot_quick_Assem_clicked(){
    Assem=new QProcess;
    Assem->start("bash");
    Assem->write("roslaunch multi_wang AssembledFormation.launch\n");
}

void MainWindow::slot_quick_leader_clicked(){
    leader=new QProcess;
    leader->start("bash");
    leader->write("roslaunch multi_scout multi_scout1.launch\n");
}

void MainWindow::slot_quick_virtual1_clicked(){
    virtual1=new QProcess;
    virtual1->start("bash");
    virtual1->write("roslaunch multi_scout multi_scout.launch\n");
}

void MainWindow::slot_quick_con_clicked(){
    con=new QProcess;
    con->start("bash");
    con->write("roslaunch multi_wang Consisience.launch\n");
}

void MainWindow::slot_quick_quit1_clicked(){
    quit1=new QProcess;
    quit1->start("bash");
    quit1->write("ps aux | grep ros |  awk '{print $2}' | xargs kill -9; ps aux | grep rviz |  awk '{print $2}' | xargs kill -9\n");
}


//关闭指定
void MainWindow::slot_quick_stop1_clicked(){
    stop1=new QProcess;
    stop1->start("bash");
    stop1->write("rosnode kill /master\n");
    stop1->write("rosnode kill /scout1_tf_broadcaster\n");
    stop1->write("rosnode kill /scout2_tf_broadcaster\n");
    stop1->write("rosnode kill /scout3_tf_broadcaster\n");
    stop1->write("rosnode kill /slave2\n");
    stop1->write("rosnode kill /slave3\n");

}

void MainWindow::slot_quick_stop2_clicked(){
    stop2=new QProcess;
    stop2->start("bash");
    stop2->write("rosnode kill /master\n");
    stop2->write("rosnode kill /scout1_tf_broadcaster\n");
    stop2->write("rosnode kill /scout2_tf_broadcaster\n");
    stop2->write("rosnode kill /scout3_tf_broadcaster\n");
    stop2->write("rosnode kill /slave2\n");
    stop2->write("rosnode kill /slave3\n");

}

void MainWindow::slot_quick_stop3_clicked(){
    stop3=new QProcess;
    stop3->start("bash");
    stop3->write("rosnode kill /Consisience_node\n");
...
void MainWindow::slot_update_dashboard(float x,float y)
{
    ...
    speed_y_dashBoard->setValue(abs(y)*1000);
