...
void RandomMapGenerate() {
  ...
  for (int i = 0; i < _obs_num; i++) {
    ...
    // if (sqrt(pow(x - _init_x, 2) + pow(y - _init_y, 2)) < 0.5) {
    //   i--;
    //   continue;
    // }

    if (sqrt(pow(x - (-1), 2) + pow(y - 0, 2)) < 0.5) {
      i--;
      continue;
    }

    if (sqrt(pow(x - (-1), 2) + pow(y - 2, 2)) < 0.5) {
      i--;
      continue;
    }
    ...
    if (sqrt(pow(x - (-3), 2) + pow(y - 0, 2)) < 0.5) {
      i--;
      continue;
    }

    if (sqrt(pow(x - (-3), 2) + pow(y - 2, 2)) < 0.5) {
      i--;
      continue;
    }
    ...
    for (int r = -widNum / 2.0; r < widNum / 2.0; r++)
      for (int s = -widNum / 2.0; s < widNum / 2.0; s++) {
        ...
        for (int t = 0; t < heiNum; t++) {              //让障碍物的高度从0开始，故设置t=0
...
int main(int argc, char** argv) {
  ...
  n.param("map/circle_num", circle_num_, 0);  //不要圆圈障碍物

  n.param("ObstacleShape/lower_rad", _w_l, 0.2);    //树形障碍物的长宽上下界
  n.param("ObstacleShape/upper_rad", _w_h, 0.3);
  n.param("ObstacleShape/lower_hei", _h_l, 0.0);
  n.param("ObstacleShape/upper_hei", _h_h, 3.0);    //树形障碍物的最高高度
