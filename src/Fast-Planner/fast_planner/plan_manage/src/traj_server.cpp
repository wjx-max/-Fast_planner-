...
void displayTrajWithColor(vector<Eigen::Vector3d> path, double resolution, Eigen::Vector4d color,
  ...
  mk.header.frame_id = frame_id_; //使用成员变量
...
void drawCmd(const Eigen::Vector3d& pos, const Eigen::Vector3d& vec, const int& id,
  ...
  mk_state.header.frame_id = frame_id_; //使用成员变量
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  cmd.header.frame_id = frame_id_; //使用成员变量
...
int main(int argc, char** argv) {
  ...
  nh.param("frame_id", frame_id_, std::string("odom"));  // 从参数服务器获取frame_id
